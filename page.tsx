...
function ProfileMenu({ onRescan, pulse = false }: { onRescan: () => void; pulse?: boolean }) {
  ...
  return (
    <div
      ...
      ref={containerRef}
      className={`relative z-50 ${swallowing ? 'profile-pill-swallow' : ''}`}
      style={{
    ...
    >
      {/* Pill header — avatar · name · token chip · chevron */}
      <button
        onClick={handleToggle}
        className="flex items-center gap-2 w-full py-1.5"
        style={{ background: 'none', border: 'none', cursor: 'pointer', paddingLeft: 7, paddingRight: 12 }}
      >
        <span className="avatar-initial">{initial}</span>
        <span
          className="font-sans text-[13px] flex-1 text-left"
...
const WALL_WONKS = ['-1.7deg', '1.3deg', '-0.9deg', '1.8deg', '-1.2deg', '0.8deg', '-1.5deg', '1.1deg'];

function ProjectCard({
  project,
  onClick,
  index = 0,
}: { project: ProjectDoc; onClick: () => void; index?: number }) {
  const [zooming, setZooming] = useState(false);
  const wonk = WALL_WONKS[index % WALL_WONKS.length];
  ...
  return (
    <div
      className={`pin-in ${zooming ? 'project-zoom' : ''}`}
      style={{ '--wonk': wonk, '--pin-delay': `${140 + index * 70}ms` } as React.CSSProperties}
    >
      <div className="wall-polaroid" style={{ position: 'relative' }}>
        <span className="pushpin" />
        <button
        onClick={handleClick}
        className="polaroid block w-full text-left"
        style={{ padding: '9px 9px 0', cursor: 'pointer', border: 'none' }}
      >
        <div style={{ aspectRatio: '1', overflow: 'hidden', borderRadius: 2, background: '#1c1510' }}>
          {project.thumbnailUrl ? (
            <img src={project.thumbnailUrl} alt={project.name} className="block w-full h-full object-cover" />
          ) : (
            <div className="w-full h-full flex items-center justify-center">
              <div style={{ width: 38, opacity: 0.35, transform: 'rotate(186deg)' }}>
                <BarberMascot isStatic />
              </div>
            </div>
          )}
        </div>
        <div className="flex items-baseline justify-between" style={{ padding: '9px 4px 11px' }}>
          <span
            className="caption-script text-[var(--char)]"
            style={{ fontSize: 17, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
          >
            {project.name}
          </span>
          <span className="font-mono" style={{ fontSize: 9, color: 'var(--smoke)', flexShrink: 0, marginLeft: 8, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
            {date}
          </span>
        </div>
        </button>
...
/* ─────────────── Add Project Button — open appointment slot ───── */
function AddProjectButton({ onClick, isEmpty }: { onClick: () => void; isEmpty?: boolean }) {
  void isEmpty; // the empty wall is handled by <EmptyChair /> in MainMenu
  return (
    <div className="pin-in" style={{ '--wonk': '0.6deg', '--pin-delay': '70ms' } as React.CSSProperties}>
      <BouncyButton
        onClick={onClick}
        className="new-cut-card w-full flex flex-col items-center justify-center gap-3"
        style={{ aspectRatio: '0.86', padding: 0 }}
      >
        <span style={{ fontSize: 38, lineHeight: 1, opacity: 0.92, fontWeight: 300 }}>＋</span>
        <span className="font-mono" style={{ fontSize: 10, letterSpacing: '0.22em', textTransform: 'uppercase', opacity: 0.8 }}>
          New cut
        </span>
      </BouncyButton>
    </div>
...
/* ─────────────── Empty state — the open chair ─────────────── */
function EmptyChair({ onScan }: { onScan: () => void }) {
  return (
    <div className="flex flex-col items-center text-center gap-6 pin-in" style={{ paddingTop: 24 }}>
      <div className="mascot-bob" style={{ width: 88 }}>
        <BarberMascot snap />
      </div>
      <h2 className="type-chonk text-[var(--cream)]" style={{ fontSize: 'clamp(2.6rem, 6vw, 4.4rem)' }}>
        The chair&rsquo;s <em style={{ color: 'var(--butter)' }}>all yours.</em>
      </h2>
      <p className="font-sans text-[var(--cream)]" style={{ fontSize: 16, opacity: 0.85, maxWidth: 400, lineHeight: 1.55 }}>
        Scan your face once, then try on every cut you&rsquo;ve ever been
        curious about — before the clippers come anywhere near you.
      </p>
      <BouncyButton onClick={onScan} className="btn btn-cream" style={{ padding: '16px 36px', fontSize: 16, fontWeight: 700 }}>
        ✂ Take your first scan
      </BouncyButton>
      <span className="font-mono" style={{ fontSize: 10, color: 'var(--cream)', opacity: 0.55, letterSpacing: '0.18em', textTransform: 'uppercase' }}>
        takes about a minute · fully reversible
      </span>
    </div>
  );
}
  ...
  return (
    <main className="relative min-h-screen bg-tomato-shop overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-6 py-4 pointer-events-none">
        <div className={`pointer-events-auto flex items-center gap-4 ${logoVisible ? 'slide-in-left' : 'opacity-0'}`}>
          <InlineWordmark cream />
          <span
            className="hidden sm:inline-flex items-center gap-2 font-mono"
            style={{ fontSize: 9, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--cream)', opacity: 0.75 }}
          >
            <span className="dot-open" /> open · walk-ins welcome
          </span>
        </div>
      ...
      {/* The wall */}
      <div className={`relative z-10 h-screen overflow-y-auto cozy-scroll ${menuVisible ? 'slide-in-left' : 'opacity-0'}`}>
        <div className="px-10 pt-24 pb-20 w-full max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <p className="font-mono" style={{ fontSize: 10, letterSpacing: '0.26em', textTransform: 'uppercase', color: 'var(--cream)', opacity: 0.65, marginBottom: 10 }}>
              fresh from the chair
            </p>
            <h1 className="type-chonk text-[var(--cream)] select-none" style={{ fontSize: 'clamp(2.6rem, 5.5vw, 4.2rem)' }}>
              My <em style={{ color: 'var(--butter)' }}>Cuts</em>
            </h1>
          </div>

          {isEmpty ? (
            <EmptyChair onScan={showScanNow ? onScanNow : onAdd} />
          ) : (
            <div
              className="grid"
              style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(185px, 1fr))', gap: '44px 38px', alignItems: 'start' }}
            >
...
export default function Home() {
  ...
  if (appState === 'hairEditLoop' && imageUrl) {
    ...
    return (
      <main className="flex fixed inset-0 overflow-hidden bg-tomato-shop">
        ...
        <div className="flex-1 min-w-0 relative studio-rise">
          {faceliftReady ? (
            <HairScene
              params={params}
        ...
        <aside className="w-80 flex-shrink-0 flex flex-col p-4 gap-4 relative overflow-hidden sidebar-in">
          <div className="flex items-center justify-between">
            <span className="inline-flex items-center gap-2">
              <span className="inline-block w-1.5 h-4 barber-pole" />
  ...
  return (
    <main className="flex fixed inset-0 overflow-hidden bg-tomato-shop">
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 pointer-events-none text-center headline-drop">
        <p className="font-mono" style={{ fontSize: 9, letterSpacing: '0.3em', textTransform: 'uppercase', color: 'var(--cream)', opacity: 0.7, marginBottom: 4 }}>
          chair no. 01 — live session
        </p>
      ...
      <div className="flex-1 min-w-0 relative flex items-center justify-center p-6 pt-28 studio-rise">
        {imageUrl && (
          <div
            className={`absolute top-28 left-6 z-10 polaroid ${previewExpanded ? '' : 'wonky-l'}`}
            style={{
        ...
        >
          ...
          <span className="absolute bottom-6 left-5 z-10 font-mono text-[10px] uppercase tracking-[0.22em] text-[var(--cream)]/70 pointer-events-none">
            live · 3d sculpt
          </span>

          {/* Backdrop dock — bottom center */}
          {!menuHidden && (
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10 swatch-dock">
              <span className="font-mono" style={{ fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--smoke)', marginRight: 2 }}>
                backdrop
              </span>
              {['#001f5b', '#000000', '#1c1510', '#00b140', '#f5f0e8'].map(c => (
                <button
                  ...
                  title={`Set backdrop to ${c}`}
                  style={{
                    background: c,
                    border: '1px solid rgba(42,32,26,0.2)',
                    boxShadow: sceneBackground === c ? '0 0 0 2px var(--ink)' : 'none',
                  }}
                />
              ))}
              <input
                ...
                style={{ width: 20, height: 20, padding: 0, border: 'none', cursor: 'pointer', borderRadius: 5, background: 'none', flexShrink: 0 }}
              />
              <span style={{ width: 1, height: 16, background: 'rgba(42,32,26,0.15)', flexShrink: 0 }} />
              <button
                onClick={() => setMenuHidden(true)}
                className="font-mono hover:text-[var(--ink)]"
                style={{ fontSize: 9, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--smoke)', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 2px' }}
              >
                hide ui
              </button>
            </div>
          ...
          {/* Re-entry point when the UI is tucked away */}
          {menuHidden && (
            <button
              onClick={() => setMenuHidden(false)}
              className="absolute bottom-4 right-4 z-10 font-mono hover:text-[var(--cream)]"
              style={{ fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(255,248,234,0.55)', background: 'rgba(0,0,0,0.35)', borderRadius: 9999, padding: '6px 14px', border: '1px solid rgba(255,248,234,0.15)', cursor: 'pointer' }}
            >
              show ui
            </button>
          )}
      ...
      {!menuHidden && <aside className="w-80 flex-shrink-0 flex flex-col p-4 gap-4 relative overflow-hidden sidebar-in">
        <div className="flex items-center justify-between">
          <span className="inline-flex items-center gap-2">
            <span className="inline-block w-1.5 h-4 barber-pole" />
            <span className="font-mono text-[10px] uppercase tracking-[0.22em] text-[var(--cream)]">the toolbox</span>
